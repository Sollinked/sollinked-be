...
import { ContentLike, fillableColumns } from "../Models/contentLike";

const table = 'content_likes';
...
export const view = async(id: number) => {
    ...
    const db = new DB();
    const result = await db.executeQueryForSingleResult<ContentLike>(query);
...
export const find = async(whereParams: {[key: string]: any}) => {
    ...
    const db = new DB();
    const result = await db.executeQueryForResults<ContentLike>(query);
