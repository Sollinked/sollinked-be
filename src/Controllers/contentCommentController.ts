...
import { ContentComment, fillableColumns } from "../Models/contentComment";
...
export const view = async(id: number) => {
    ...
    const db = new DB();
    const result = await db.executeQueryForSingleResult<ContentComment>(query);
...
export const find = async(whereParams: {[key: string]: any}) => {
    ...
    const db = new DB();
    const result = await db.executeQueryForResults<ContentComment>(query);
